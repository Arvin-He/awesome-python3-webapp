...
def create_pool(loop, **kw):
    ...
    __pool = yield from aiomysql.create_pool(
        ...
        db=kw['db'], # 数据库名
        charset=kw.get('charset', 'utf-8'), # 设置连接使用的编码格式utf-8
...
def select(sql, args, size=None):
    ...
    global __pool
    # 从连接池中获取一个连接
    with (yield from __pool) as conn: # with...as...的作用相当于try...exception...
...
class ModelMetaclass(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        # 取table名称，若没有定义__table__属性，将类名作为表名，此处注意or的用法
        tableName = attrs.get('__table__', None) or name
        logging.info('found model: %s (table: %s)' % (name, tableName))
        # 获取所有的Feild和主键名
        mappings = dict() # 用字典来存储类属性与数据库的列表的映射关系
        fields = [] # 用于保存除主键以外的属性
